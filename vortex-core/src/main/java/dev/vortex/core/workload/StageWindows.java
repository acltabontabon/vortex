...
public final class StageWindows {
     ...
     */
    public record StageWindow(int index, dev.vortex.core.shared.LoadLevel target, TimeWindow window,
     ...
     */
    public static dev.vortex.core.shared.LoadLevel levelAt(List<Stage> stages, Duration offset) {
        if (stages == null || stages.isEmpty() || offset == null || offset.isNegative()) {
            return null;
        }
        Duration cursor = Duration.ZERO;
        dev.vortex.core.shared.LoadLevel current = null;
    ...
    public static List<StageWindow> fromObservedVirtualUsers(List<Stage> stages,
            List<dev.vortex.core.metrics.SamplePoint> samples) {

        if (stages == null || stages.isEmpty() || samples == null || samples.isEmpty()) {
            return List.of();
        }
        boolean everyStageTargetsVirtualUsers = stages.stream()
                .allMatch(stage -> stage.target() instanceof dev.vortex.core.shared.Concurrency);
        ...
        for (int index = 0; index < stages.size(); index++) {
            Stage stage = stages.get(index);
            int target = ((dev.vortex.core.shared.Concurrency) stage.target()).vus();

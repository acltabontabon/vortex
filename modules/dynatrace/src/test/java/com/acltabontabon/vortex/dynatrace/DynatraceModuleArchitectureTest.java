 ...
 *
 * <p>{@link DynatraceMcpConfigImport} parses a config an SRE team hands out and extracts a remote
 * URL from it — never a command. This rule makes that a build-time guarantee rather than a
 * convention someone could accidentally erode with a future "helper" that shells out.
 */
@AnalyzeClasses(
    ...
    static final ArchRule this_module_never_spawns_a_process =
            noClasses().should().dependOnClassesThat().belongToAnyOf(ProcessBuilder.class, Runtime.class)
                    .because("""
                            ...
                            runs the npx/mcp-remote command an SRE-shared config describes. Spawning a \

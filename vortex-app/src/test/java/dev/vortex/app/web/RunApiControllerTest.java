...
class RunApiControllerTest {
    ...
    static class RealEvidence {
        ...
        dev.vortex.core.application.RunEvidenceService runEvidenceService() {
            return new dev.vortex.core.application.RunEvidenceService(
                    new dev.vortex.core.application.DeterministicAnalyzer(new ThresholdEvaluator(),
                            new dev.vortex.core.analysis.BreakpointDetector(),
                            new dev.vortex.core.analysis.SystemSaturationDetector()),
                    new dev.vortex.core.evidence.FindingDetector(),
                    new dev.vortex.core.evidence.EvidenceSanitizer(),
    ...
    private TestExecution sparse() {
        ...
        var bare = new dev.vortex.core.metrics.MeasuredResults(
                new dev.vortex.core.metrics.TimeWindow(Fixtures.NOW, Fixtures.NOW.plusSeconds(600)),
                null, null, 10, 0, dev.vortex.core.metrics.LatencyPercentiles.empty(),
                java.util.Map.of(), null, List.of());

        var summary = new DeterministicSummary("Does it work?",
                dev.vortex.core.threshold.Verdict.NOT_EVALUATED, "Not established.", bare,
                dev.vortex.core.threshold.ThresholdEvaluation.empty(), null, null, List.of());
    ...
    class Preflight {
        ...
        void reportsWhatWillHappen() throws Exception {
            var plan = Fixtures.plan();
            var report = new dev.vortex.core.application.PreflightReport(plan, List.of(),
                    dev.vortex.core.safety.SafetyAssessment.clear());
        ...
        void withoutAWorkloadOrEnvironment() throws Exception {
            when(projects.configuration(any())).thenReturn(dev.vortex.core.project.ProjectConfiguration.empty());
    ...
    class StartingARun {
        ...
        void needsConfirmation() throws Exception {
            var plan = Fixtures.plan();
            var report = new dev.vortex.core.application.PreflightReport(plan, List.of(),
                    new dev.vortex.core.safety.SafetyAssessment(List.of(
                            dev.vortex.core.safety.SafetyFinding.challenge("mutating-ops",
